...
`go-ruby-etcd/etcd` reimplements Ruby's `etcd` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-etcd/etcd` directly, with no Ruby runtime;
